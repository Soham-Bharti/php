...
if ($_SESSION['role'] == 'emp') {
    ...
    $sql1 = "SELECT date(check_out_time) as date from employeeTrackingDetails where user_id = '$desiredUserId' and deleted_at is null order by check_out_time desc limit 1;";
    $result1 = mysqli_query($conn, $sql1);
    if (mysqli_num_rows($result1) > 0) {
        $row1 = mysqli_fetch_assoc($result1);
        $checkOutDate = $row1['date'];
        echo $checkOutDate;
